---
sidebar: sidebar 
permalink: administer/additional-ontap-cluster.html 
keywords: afx, afx system, additional administration, advanced administration 
summary: 除了典型的 AFX 叢集管理之外，您可能還需要根據您的環境執行其他任務。大多數附加任務都可以使用系統管理員執行，但在某些情況下您可能需要使用 CLI。 
---
= AFX 儲存系統叢集的附加管理
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
除了典型的 AFX 叢集管理之外，您可能還需要根據您的環境執行其他任務。大多數附加任務都可以使用系統管理員執行，但在某些情況下您可能需要使用 CLI。


NOTE: 所描述的ONTAP功能和管理對於運行 Unified ONTAP 的AFX 儲存系統和AFF或FAS系統來說是通用的。其中包含相關 Unified ONTAP文件的連結（視情況而定）。



[[licensing]]
== 授權

AFX 系統的授權方式與 Unified ONTAP AFF和FAS系統類似。  AFX 叢集預設包含所支援協定的大多數功能。



[[ontap-license-management]]
=== ONTAP許可證管理

ONTAP許可證是一個或多個軟體授權的記錄。所有許可證均使用NetApp許可證文件 (NLF) 定義和提供。參考 https://docs.netapp.com/us-en/ontap/system-admin/manage-licenses-concept.html["ONTAP許可概述"^]了解更多。



[[install-a-license-on-an-afx-system]]
=== 在 AFX 系統上安裝許可證

您可以安裝許可證檔案來根據需要啟動 AFX 儲存系統的附加功能。

.步驟
. 在系統管理員中，選擇*集群*，然後選擇*設定*。
. 在「許可證」旁邊，選擇image:icon_arrow.gif["指向右側的藍色箭頭"]。
. 選擇“*功能*”標籤以顯示可用的ONTAP功能。
. 若要選擇性地安裝許可證，請選擇「*已安裝的許可證*」標籤。
. 選擇image:icon_add_blue_bg.png["藍色矩形框內有一個加號，後面跟著白色字母“add”"]。
. 選擇本機授權檔案並選擇*新增*。




[[performance]]
== 效能

利用 ONTAP 的常用功能來優化 AFF 儲存系統叢集的效能。



[[modify-default-settings-that-control-automatic-qos-throughput-ceiling-increases]]
=== 修改控制自動 QoS 吞吐量上限增加的預設設定

預設情況下，自適應和非自適應 QoS 原則群組會自動啟用，以便在短期內將工作負載的處理量上限暫時提高 50%，持續 1 秒，從而回應對更高效能的短期需求。從 ONTAP 9.19.1 開始，您可以修改這些預設設定。在 ONTAP 9.18.1 及更早版本中，這些設定無法變更。

請參閱link:https://docs.netapp.com/us-en/ontap/performance-admin/guarantee-throughput-qos-task.html["QoS 吞吐量上限"^]。



[[ipspace-tcp-performance]]
=== IPspace TCP 效能

從 ONTAP 9.19.1 版本開始，ONTAP 透過在 ONTAP TCP 堆疊中整合 PRR 和 RACK 兩項丟包恢復功能，提升了在丟包率高、延遲高的 WAN 連結上的 TCP 資料傳輸效能。PRR 適用於所有網路，預設為啟用。RACK 適用於連結速度約為 25 Gigabits/second 的擁塞網路，預設為停用。

請參閱 link:https://docs.netapp.com/us-en/ontap/networking/improve-tcp-wan-performance.html["利用 PRR 和 RACK 提升 TCP WAN 效能"] 以取得更多資訊。



[[security]]
== 安全

您可以在 AFX 部署中設定和使用多種可選的安全功能。



[[ontap-security-and-data-encryption]]
=== ONTAP安全性與資料加密

保護 AFX 儲存系統的安全性和隱私性非常重要。請參閱 https://docs.netapp.com/us-en/ontap/security-encryption/index.html["安全性和資料加密"^]



[[ontap-authentication-and-access-control]]
=== ONTAP身份驗證和存取控制

AFX 儲存系統提供了多種設定驗證和存取控制服務的選項。參考 https://docs.netapp.com/us-en/ontap/authentication-access-control/index.html["身份驗證和存取控制"^]了解更多。



[[ontap-tls-hardware-offload]]
=== ONTAP TLS 硬體卸載

您可以將整個叢集的傳輸層安全性（TLS）加密和解密任務卸載到支援的乙太網路卡上。這可以降低 CPU 開銷，並提高使用 TLS 的系統的效能。如需詳細資訊，請參閱 https://docs.netapp.com/us-en/ontap/networking/configure-tls-offload.html["設定 ONTAP TLS 硬體卸載"^]。



[[administer-oauth-2-0-on-an-afx-system]]
=== 在 AFX 系統上管理 OAuth 2.0

OAuth 2.0 是業界標準授權框架，用於使用簽署的存取權杖來限制和控制對受保護資源的存取。

.步驟
. 在系統管理員中，選擇*集群*，然後選擇*設定*。
. 在「安全」部分中，在「OAuth 2.0 授權」旁邊，選擇image:icon_arrow.gif["指向右側的藍色箭頭"]。
. 啟用 OAuth 2.0
. 選擇*新增配置*並提供配置詳細資訊。
. 選擇*儲存*。




[[data-protection]]
== 資料保護

您應該瞭解以下 ONTAP 資料保護功能，以及這些功能與 AFX 儲存系統的關係。



[[load-share-mirrors]]
=== 負載共享鏡射

在 ONTAP 中，負載共用鏡射 （LSM） 是儲存 VM （SVM） 根 Volume 的 SnapMirror 複本，可作為 SVM NAS 命名空間的入口點。透過在其他節點上維護一個或多個根 Volume 的唯讀複本，負載共用鏡射有助於確保即使 HA 配對的兩個節點都無法使用，用戶端仍可存取 NAS 命名空間。

*AFX 儲存系統不支援負載平衡鏡射。*您無法在 AFX 上建立、初始化或更新負載平衡鏡射關係。若要在 AFX 上實現命名空間彈性、資料經銷或讀取擴充，請改用 FlexCache® 磁碟區或 SnapMirror 複寫。這些功能可提供同等或更強大的功能，讓用戶端能夠存取資料和命名空間，且 AFX 完全支援這些功能。



[[related-information]]
== 相關資訊

* link:../faq-ontap-afx.html["AFX 儲存系統常見問題解答"]
* https://kb.netapp.com/nss/Licensing/How_to_download_NLF_licenses_from_the_NetApp_Support_Site["如何從 NetApp 支援網站下載 NLF 授權"^]
* https://docs.netapp.com/us-en/ontap/authentication/overview-oauth2.html["ONTAP OAuth 2.0 實作概述"^]
* link:../administer/additional-ontap-svm.html["AFX SVM 的額外管理"]
* https://docs.netapp.com/us-en/ontap/data-protection/manage-snapmirror-root-volume-replication-concept.html["了解 ONTAP SnapMirror 根 Volume 複寫"^]

