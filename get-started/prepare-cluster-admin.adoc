...
NOTE: 從 ONTAP 9.19.1 開始、SVM 管理員可以使用專用的 System Manager 儀表板來監控和管理自己的 SVM。深入瞭解 https://docs.netapp.com/us-en/ontap/concept-svm-admin-dashboard-sm.html["ONTAP System Manager SVM 管理員儀表板"^]
...
AFX 在設定和使用叢集時會強制執行一些限制，包括最小值和最大值。這些限制分為以下幾類。


CAUTION: 在提升 AFX 儲存系統的任何運作參數之前，您應該評估其對應用程式以及整個叢集的影響。例如，增加控制器節點數量雖然可以提高整體容量，但也可能會增加叢集網路的流量。因此，了解這些組態變更的影響以及它們可能對您的環境造成的影響至關重要。

每個叢集的控制器節點:: 每個 AFX 叢集必須至少有四個節點。最大節點數根據 ONTAP 版本而有所不同；例如，9.19.1 版本允許每個 AFX 叢集最多有 32 個節點。
儲存容量:: 這是叢集儲存可用區 (SAZ) 中所有 SSD 磁碟的總容量。最大儲存容量根據ONTAP版本而有所不同。
集群交換機:: 叢集儲存網路中至少需要兩台交換器。允許的最大值是根據叢集中控制器節點的總數來決定的。


您應查看 NetApp Hardware Universe 和互通性矩陣工具中提供的詳細資訊，以確定您的 AFX 叢集的功能。另請參閱 AFX 發行說明部分中的link:../release-notes/changes-to-limits-defaults.html["ONTAP 限制和預設值的變更"]。
