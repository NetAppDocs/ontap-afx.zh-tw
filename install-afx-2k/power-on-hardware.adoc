...
[role="lead"]
安裝 AFX 2K 儲存系統的機架硬體並安裝控制器節點和儲存架的纜線後，您應該開啟儲存架和控制器節點的電源。
...
NOTE: 由於 Cisco Nexus 9808 交換器是單一交換器，ISL 已中斷；因此，您將在命令輸出中看不到對等體。
...
.下一步是什麼？
啟動 AFX 2K 儲存系統後，根據您的部署選項完成設定：

* 對於專用 AFX 2K 儲存系統部署，請前往link:../install-setup/cluster-setup.html["設定 AFX 叢集"]。
