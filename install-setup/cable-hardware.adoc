...
* NVIDIA C6X-DX 和 CX7 （100GbE） 網路卡不支援銅纜和光纖 breakout 連接（纜線）混合使用。您應該只使用銅纜或光纖連接，避免混合不同類型的連接。長時間混用不同類型的連接可能會導致鏈路飽和。
+
在連接交換器之前，請確保您擁有與連接類型相符的正確線纜。

+

NOTE: 有關混合連線類型的詳細資訊，請參閱 NetApp 知識庫文章link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CHW-3504["在 CX6-DX NIC 上使用混合銅纜和光纖分支線連接連接埠時，出現 CRC 連結錯誤"^]。
...
+
image::../media/drw_afx_ha_cluster_storage_a_ieops-2779.svg[電纜控制器叢集,800px,and storage to switch A]
...
+
image::../media/drw_afx_ha_cluster_storage_b_ieops-2780.svg[電纜控制器叢集,800px,and storage to switch B]
