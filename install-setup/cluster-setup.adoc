...
summary: 安裝完 AFX 硬體後，即可完成 ONTAP 叢集的設定。此過程包含多個相關的配置階段，您需要依序執行這些階段。 
---
= 設定 AFX 儲存系統ONTAP集群
:allow-uri-read: 
:icons: font
...
[role="lead"]
安裝完 AFX 硬體後，即可完成 ONTAP 叢集的設定。此過程包含多個相關的配置階段，您需要依序執行這些階段。
...
在設定 ONTAP 叢集之前，您應該查看 AFX 核心需求，以確保您擁有成功完成設定流程所需的所有資訊和資源。
